...
export default function NonConformities() {
  ...
  fetch("http://localhost:5000/api/nonconformities", {
    ...
    body: JSON.stringify(nonConformityWithId)
  })
    .then(res => res.json())
    .then(saved => {
  setNonConformities([...nonConformities, saved]);

addActionFromNonConformity({
  nonConformity: saved,
  responsible: saved.responsible,
  correctionDate: saved.dueDate,
  recommendation: saved.correctiveAction
});
fetch("http://localhost:5000/api/actions", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    action: "Action liée à une non-conformité",
    responsible: saved.responsible,
    correctionDate: saved.dueDate,
    recommendation: saved.correctiveAction,
    status: "À faire",
    nonConformities: [saved], // ou [saved.title] si tu préfères afficher juste le titre
  }),
})
  .then(res => res.json())
  .then(data => {
    console.log("✅ Action liée enregistrée avec succès :", data);
  })
  .catch(err => console.error("❌ Erreur enregistrement action :", err));
