...
React.useEffect(() => {
  fetch("http://localhost:5000/api/actions")
    .then((res) => res.json())
     .then((data) => {
      console.log("✅ Actions FETCHED:", data);  // 🔍 regarde ici si tu as des NC peuplées
...
const renderNonConformitiesNames = (items) => {
 ...
 const handleAddSubmit = (e) => {
  ...
  fetch(`http://localhost:5000/api/actions/${currentAction._id || currentAction.id}`, {
 ...
 setFetchedActions((prev) =>
  prev.map((a) =>
    (a._id || a.id) === (data._id || data.id) ? data : a
  )
);
      updateAction(data); // Si contexte
      closeModals();
    })
...
const handleDelete = (action) => {
  if (window.confirm("Voulez-vous vraiment supprimer cette action ?")) {
    fetch(`http://localhost:5000/api/actions/${action.id}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (!res.ok) throw new Error("Erreur serveur");
        setFetchedActions((prev) => prev.filter((a) => a.id !== action.id));

        deleteAction(action.id); // Si besoin contexte
      })
      .catch((err) => {
      ...
      <div className="block w-full overflow-x-auto">
        {[...fetchedActions, ...contextActions].length === 0 ? (
          ...
          <table className="items-center w-full bg-transparent border-collapse">
            ...
            <tbody>
{[...fetchedActions, ...contextActions].map((action, indexAction) => {
  console.log(`Action ${indexAction} nonConformities:`, action.nonConformities);

  return (
    <tr key={`action-${action._id || action.id || indexAction}`} className="hover:bg-blueGray-50">
      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
  {Array.isArray(action.nonConformities) && action.nonConformities.length > 0 ? (
    <ul className="list-disc list-inside space-y-1">
      {action.nonConformities.map((nc) => {
        console.log("🟢 nonConformity = ", nc); // log ok

        return (
          <li key={nc._id} className="flex items-center justify-between">
            <span>{nc.title || nc.name || "Titre inconnu"}</span>
            <button
              onClick={() => openAddModalWithNonConformity(nc)}
              className="text-green-600 hover:text-green-800 font-semibold text-xs px-2 py-1 border border-green-600 rounded"
    ...
    : "-"}
      ...
      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={openAddModal}
            className="text-blueGray-500 hover:text-blueGray-700 p-2 font-bold"
            title="Ajouter une action"
          >
